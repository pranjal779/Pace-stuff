...
DROP FUNCTION IF EXISTS dvc;
DELIMITER //

create function dvc(D CHAR (30))
RETURNS CHAR(30)
DETERMINISTIC
...
BEGIN
	...
	DECLARE WI_ID, atp, NETW_ass,w_loc, w_access,witype VARCHAR(30);
	DECLARE w_sec INT;
	DECLARE Loca varchar(100);
	Declare code,sec_cod, s_code,ne_type VARCHAR(40);

	-- DECLARE CURSOR
	DECLARE WID_CURSOR CURSOR FOR SELECT wid, WType, assignedto, location, AccessCode, secure from widget;
	...
	OPEN WID_CURSOR;
		...
		WHILE I < count_rows DO
			FETCH WID_CURSOR INTO WI_ID, atp, NETW_ass, w_loc, w_access, w_sec;
			set CODE = acc(w_access);
			set sec_cod = sec(w_sec);
			set witype = dvc(atp);
			set loca = R_loc(w_loc);
			set ne_type = substring_index(NETW_ass, '_', -1);
			...
			INSERT INTO R_widget(WidgetID, RTYPE, NetAssigned, RNetType, RLocation, RAccess, Rsecure, RUser) VALUES
			(WI_ID, witype, NETW_ass, ne_type, loca, code, sec_cod, JSON_OBJECT("ID:",(WI_ID),"Device:",(atp),"Access Code:", (w_access), "EncryptedYN", (s_code)));
			SET I = I + 1;
		END WHILE;
