...
Declare count_rows, I,J, Length int;

-- Create Cursor Variables
DECLARE w_ide, w_net, w_acc, n_stat, s_nam, w_ids, w_accs, n_stats, b VARCHAR(150);
...
WHILE I < count_rows DO
	...
	SET I = I + 1;
	-- Select * from widgetAnalysis_1 where (w_id) = substring_index(b,'~',1);
END WHILE;

	set J = 0;
	set length = JSON_LENGTH(A);
	select length;
	while J < length DO
...
select * from widgetAnalysis_1 where w_id = trim(leading '"' from substring_index(b,'~',1));
